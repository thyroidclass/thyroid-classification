...
class Argments(object):
        ...
        for k, v in self._y.items():
            if 'module' in k:
                ...
                if self['setup/phase'] != 'infer':
                    ...
                    module['model'] = torch.nn.DataParallel(module['model'])
    ...
    def __init__(self, yaml_file, cmd_args):
        ...
        model_path = f"outs/{self['path/dir_name']}_{self['setup/model_type']}_{self['setup/num_mc']}"
        model_path += f"_{self['path/dataset']}"
